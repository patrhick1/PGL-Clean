...
from src.auth_middleware import get_admin_user

# Import the test-specific AI usage tracker
from test_ai_usage_tracker import tracker
...
def run_test_thread(test_id, workflow, model, test_name, limit=None, batch_size=5, concurrency=3):
    ...
    try:
        ...
        # In a non-main thread we can't use signal handlers directly
        # Create a flag that can be checked periodically to allow for graceful shutdown
        active_tests[test_id]['should_stop'] = False
        
        try:
            # Run the test function
            if hasattr(module, 'run_test'):
                logger.info(f"Running test with kwargs: {kwargs}")
                # Pass the stop flag to the test function if it accepts it
                if 'stop_flag' in module.run_test.__code__.co_varnames:
                    kwargs['stop_flag'] = lambda: active_tests[test_id].get('should_stop', False)
                ...
                from test_determine_fit import run_test
...
async def stop_test(test_id: str, user: dict = Depends(get_admin_user)):
    ...
    try:
        ...
        # Set the should_stop flag to signal the thread to stop gracefully
