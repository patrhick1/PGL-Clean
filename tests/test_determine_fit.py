...
import pandas as pd
from airtable_service import AirtableService # Assuming this will be found if project root is in sys.path
...
from test_ai_usage_tracker import tracker as test_tracker  # Adjusted path
...
async def run_test(batch_size: int, concurrency: int, limit: Optional[int], output_file: str,
    ...
    # Modified to check the stop flag
    class StoppableProcessor(TestFitProcessor):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.should_stop = False
            
        async def process_all_records(self, *args, **kwargs):
            original_process_batch = self.process_batch
            
            async def stoppable_process_batch(batch, *args, **kwargs):
            ...
            self.process_batch = stoppable_process_batch
            result = await super().process_all_records(*args, **kwargs)
            
            # Check at the end too in case process_all_records is slow to return
            if stop_flag and stop_flag():
                logger.info("Stop flag detected after processing")
                
            return result
    
    # Use the stoppable processor if a stop flag was provided, otherwise use the regular one
    if stop_flag:
        processor = StoppableProcessor(model=model, test_name=test_name)
