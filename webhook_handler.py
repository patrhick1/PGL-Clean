...
def poll_podcast_search_database(record_id, stop_flag: Optional[threading.Event] = None):
    ...
    try:
        ...
        try:
            ...
            process_mipr_podcast_search_with_podscan(record_id, stop_flag)
        except Exception as e:
            logger.error(f"Error processing record {record_id}: {e}")
    except Exception as e:
...
async def process_single_podcast(podcast: Dict, openai_service: OpenAIService) -> Dict[str, Any]:
    ...
    try:
        ...
        # Prepare the prompt
        raw_text = (f"Podcast Name: {podcast_name}\n"
...
async def enrich_host_name_async(stop_flag: Optional[threading.Event] = None, 
    ...
    try:
        ...
        stats_file = f"host_name_enrichment_stats_{timestamp}.json"
        
        with open(stats_file, 'w') as f:
            # Create a copy of stats that's JSON serializable
            json_stats = stats.copy()
            json_stats['start_time'] = datetime.fromtimestamp(start_time).isoformat()
            json_stats['end_time'] = datetime.fromtimestamp(time.time()).isoformat()
            
            # Remove the full results array to keep the file size manageable
            detailed_results = json_stats.pop('results')
            
            # Add summary of results
            json_stats['results_summary'] = {
                'successful': stats['successful'],
                'failed': stats['failed'],
                'success_rate': f"{stats['successful']/max(stats['total_processed'], 1)*100:.1f}%"
            }
