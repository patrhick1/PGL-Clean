...
async def determine_fit_async(stop_flag: Optional[Any] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        stats = await processor.process_all_records(max_concurrency=3, batch_size=5, stop_flag=stop_flag)
...
def determine_fit(stop_flag: Optional[Any] = None) -> Dict[str, Any]:
    """
    ...
    Returns:
        Dictionary with processing statistics
    """
    return asyncio.run(determine_fit_async(stop_flag))
...
if __name__ == "__main__":
    ...
    results = asyncio.run(determine_fit_async())
