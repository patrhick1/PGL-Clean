...
class PitchWriterProcessor:
    ...
    def _extract_google_doc_id(self, url: str) -> Optional[str]:
        ...
        if not url:
            return None
        # Regex to find the document ID in various Google Docs URL formats
    ...
    def _create_subject_line_template(self) -> PromptTemplate:
        ...
        The subject line should be: "Great episode about [topic]"

        Here is a summary of the episode, determine what the topic was and include it in your subject line:
        {episode_summary}
        {ai_summary}

        Give ONLY one subject line as your only output.
        """
    ...
    async def _run_pitch_email_generation(
        ...
        media_kit_url: str,
        pitch_topics: str,
        media_kit_content: Optional[str],
        podcast_id: str
    ...
    async def process_single_record(self, cm_record_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            media_kit_url = campaign_fields.get('Media Kit URL', '')
            ...
            # Get Media Kit Content
            media_kit_content_str = "No content available"
            if media_kit_url:
                doc_id = self._extract_google_doc_id(media_kit_url)
                if doc_id:
                    try:
                        logger.info(f"Fetching content from Google Doc ID: {doc_id} for media kit.")
                        media_kit_content_str = self.google_docs_service.get_document_content(doc_id)
                        if not media_kit_content_str:
                             media_kit_content_str = "No content available (document was empty or unreadable)."
                        logger.info(f"Successfully fetched media kit content (length: {len(media_kit_content_str)} chars).")
                    except Exception as e:
                        logger.warning(f"Failed to fetch/read Google Doc ({media_kit_url}) content: {e}")
                        media_kit_content_str = f"Error fetching media kit: {e}" # Provide error context to LLM
                else:
                    logger.warning(f"Could not extract Google Doc ID from URL: {media_kit_url}")
                    media_kit_content_str = "Media kit URL was invalid or ID could not be extracted."
            else:
                logger.info("No media kit URL provided.")
            ...
            pitch_email, email_tokens, email_exec_time = await self._run_pitch_email_generation(
                ...
                pitch_topics=pitch_topics,
                media_kit_content=media_kit_content_str,
                podcast_id=podcast_id
            )
    ...
    async def process_all_records(self, max_concurrency=1, batch_size=1) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Pitch writer completed. Success: {stats['successful']}/{stats['total_records']}")
...
async def pitch_writer_async(stop_flag: Optional[Any] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Change max_concurrency from 2 to 1 to avoid rate limits
        return await processor.process_all_records(max_concurrency=1, batch_size=2)
