...
async def transcribe_endpoint_free_tier(
    audio_url: str,
...
async def get_podcast_audio_transcription_free_tier(stop_flag=None):
    airtable_service = PodcastService()
    records = airtable_service.search_records("Podcast_Episodes", view = "abc")
    for record in records:
        # Check if the stop flag is set
        if stop_flag and stop_flag.is_set():
            logger.info("Stop flag detected, halting podcast transcription")
            break
        ...
        if not audio_url:
            ...
            logger.info(f"Found audio URL for record {record_id}: {audio_url}")

        try:
            transcript_result = await transcribe_endpoint_free_tier(audio_url, episode_name=record['fields'].get('Episode Title', ''))
            ...
            if error_type == "Content Policy Violation":
                ...
                # Update the record with the potentially truncated transcript
            ...
            # Catch exceptions outside of transcribe_endpoint_free_tier or during Airtable update
            logger.error(f"Failed to process record {record_id} (exception outside endpoint): {e}", exc_info=True)

        # Check stop flag again after processing each record
        if stop_flag and stop_flag.is_set():
            logger.info("Stop flag detected after processing record, halting podcast transcription")
            break

if __name__ == "__main__":
    asyncio.run(get_podcast_audio_transcription_free_tier())
